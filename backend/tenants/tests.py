...
class TenantMiddlewareResolutionTest(TestCase):
    ...
    def test_subdomain_refuses_a_tenant_awaiting_activation(self):
        """A school that has registered but is not activated yet is not reachable."""
        make_tenant("pending-school", "pending", False)
        self.assertIsNone(self.run_request(host=f"pending-school.{PLATFORM_DOMAIN}"))

    def test_subdomain_refuses_status_drift(self):
        """
        A tenant is reachable only when status='active' AND is_active. Either
        one alone used to be enough on some branches, so each half is checked.
        """
        make_tenant("drift-status", "suspended", True)
        make_tenant("drift-flag", "active", False)
        self.assertIsNone(self.run_request(host=f"drift-status.{PLATFORM_DOMAIN}"))
        self.assertIsNone(self.run_request(host=f"drift-flag.{PLATFORM_DOMAIN}"))
