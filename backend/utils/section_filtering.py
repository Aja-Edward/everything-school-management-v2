...
class SectionFilterMixin:
    ...
    def get_user_education_level_access(self):
        ...
        logger.info(f"🎓 Getting education level access for {user.username}")

        # ============================================
        # CRITICAL FIX: Check superuser/staff FIRST
        # ============================================

        # Check 1: Direct is_superuser check
        ...
        # Check 3: Role-based check
        role = self.get_user_role()
        logger.info(f"📋 User role: {role}")

        if role == "superadmin":
            logger.info("✅ Super admin (role) - all education levels")
            return ["NURSERY", "PRIMARY", "JUNIOR_SECONDARY", "SENIOR_SECONDARY"]
    ...
    def apply_section_filters(self, queryset):
        ...
        logger.info(f"🔍 Applying filters for {user.username} on {model_name}")

        # ============================================
        # CRITICAL FIX: Multiple safety checks for superadmin/staff
        # ============================================

        # Check 1: Direct is_superuser check
        if getattr(user, "is_superuser", False):
            logger.info(
                f"✅ Super admin (is_superuser=True) - no filtering on {model_name}"
            )
            return queryset
        ...
        # Check 3: Role-based check
        role = self.get_user_role()
        logger.info(f"📋 User role detected: {role}")

        if role == "superadmin":
            logger.info(
                f"✅ Super admin (role=superadmin) - no filtering on {model_name}"
            )
            return queryset

        # Check 4: General admins/principals see everything
        if role in ["admin", "principal"]:
            logger.info(
                f"✅ Admin/Principal (role={role}) - no filtering on {model_name}"
            )
            return queryset
        ...
        if not allowed_education_levels:
            logger.warning(
                f"❌ No education level access for {user.username} (role={role})"
            )
