...
qs = Tenant.objects.filter(slug=args.slug) if args.slug else Tenant.objects.all()
...
for tenant in qs:
    ...
    for code, items in exam_by_code.items():
        if len(items) > 1:
            ...
            for i, e in enumerate(items, 1):
                print(f"    {i}. id={e.id} name={e.name!r} created_at={e.created_at}")
    if not any_dup:
        print('  None')
    ...
    any_dup = False
    for lvl in levels:
        comps = AssessmentComponent.objects.filter(tenant=tenant, education_level=lvl).order_by('name', 'created_at')
        ...
        if lvl_dups:
            ...
            for k, items in lvl_dups.items():
                ...
                for i, c in enumerate(items, 1):
                    print(f"      {i}. id={c.id} code={c.code!r} name={c.name!r} created_at={c.created_at}")
    if not any_dup:
        print('  None')
