...
print('StudentResult grade status:', list(StudentResult.objects.filter(tenant=tenant).values('status').annotate(c=Count('id'))))
print('StudentResult distinct students:', StudentResult.objects.filter(tenant=tenant).values('student').distinct().count())
...
for model_name in ['NurseryResult', 'PrimaryResult', 'JuniorSecondaryResult', 'SeniorSecondaryResult']:
    try:
        ...
        print(f'{model_name} count:', model.objects.filter(tenant=tenant).count())
    except LookupError:
        print(f'{model_name} not found')

for r in StudentResult.objects.filter(tenant=tenant)[:10]:
    print('SR', r.id, r.student_id, r.subject_id, r.grade, r.status, r.total_score)
