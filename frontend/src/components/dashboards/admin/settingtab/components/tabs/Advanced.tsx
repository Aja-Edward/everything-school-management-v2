...
// import {
//   Megaphone, Users, Plus, Edit3, Trash2, Eye, EyeOff,
//   Save, X, Upload, Star, Sparkles,
//   Loader2, CheckCircle, AlertCircle,
//   Image as ImageIcon, Radio, LayoutTemplate,
//   Zap, Bell
// } from 'lucide-react';
...
// Fields that are irrelevant (and should not be sent) when display_type is ribbon
const RIBBON_EXCLUDED_FIELDS = ['subtitle', 'description', 'cta_text', 'cta_url'] as const;
...
const EventForm: React.FC<{
  ...
  // FIX: When switching to ribbon, immediately clear fields that are
  // not applicable to ribbons so they are never sent to the backend.
  const upd = (patch: Partial<typeof form>) =>
    setForm(f => {
      const next = { ...f, ...patch };
      if (patch.display_type === 'ribbon') {
        next.subtitle    = '';
        next.description = '';
        next.cta_text    = '';
        next.cta_url     = '';
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const isRibbon = form.display_type === 'ribbon';

    try {
      let result: EventItem;
      if (isEdit) {
        // PATCH text fields — exclude ribbon-irrelevant fields when display_type is ribbon
        result = await api.patch(`/events/events/${initial!.id}/`, {
          title:            form.title,
          subtitle:         isRibbon ? '' : form.subtitle,
          description:      isRibbon ? '' : form.description,
          display_type:     form.display_type,
          event_type:       form.event_type,
          background_theme: form.background_theme,
          ribbon_text:      form.ribbon_text,
          ribbon_speed:     form.ribbon_speed,
          start_date:       form.start_date || null,
          end_date:         form.end_date || null,
          cta_text:         isRibbon ? '' : form.cta_text,
          cta_url:          isRibbon ? '' : form.cta_url,
          is_active:        form.is_active,
          is_published:     form.is_published,
        });
        // Upload image separately if provided (not applicable for ribbon, but guard anyway)
        if (imageFile && !isRibbon) {
          const fd = new FormData();
          fd.append('event', String(initial!.id));
          fd.append('image', imageFile);
          await fetch(`${API_BASE_URL}/events/event-images/`, {
        ...
        // POST via FormData to support image upload.
        // For ribbon events, skip fields that aren't relevant so the backend
        // doesn't receive (or attempt to validate) them.
        const fd = new FormData();
        Object.entries(form).forEach(([k, v]) => {
          const isRibbonExcluded =
            isRibbon && (RIBBON_EXCLUDED_FIELDS as readonly string[]).includes(k);
          if (!isRibbonExcluded && v !== '' && v != null) {
            fd.append(k, String(v));
          }
        });
        if (imageFile && !isRibbon) fd.append('images', imageFile);
        const res = await fetch(`${API_BASE_URL}/events/events/`, {
          method: 'POST', body: fd, credentials: 'include',
          headers: { 'X-CSRFToken': csrfToken() },
        });
  ...
  return (
    <form onSubmit={submit} className="space-y-5 bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
      ...
      {/* Subtitle + Description — not shown or sent for ribbon */}
      {form.display_type !== 'ribbon' && (
        <>
          <Field label="Subtitle">
            <Inp value={form.subtitle} onChange={e => upd({ subtitle: e.target.value })} placeholder="Optional subtitle shown below title" />
      ...
      {/* Image upload (banner + carousel only) */}
      {form.display_type !== 'ribbon' && (
        <Field label="Event Image">
          <div className="flex gap-3 items-start">
            {imagePreview && (
      ...
      {showForm && (
        <EventForm
          initial={editTarget ? {
            ...
            display_type: editTarget.display_type,
            event_type:   editTarget.event_type,
            ribbon_text:  editTarget.ribbon_text ?? '',
            ribbon_speed: editTarget.ribbon_speed ?? 'medium',
            start_date:   editTarget.start_date?.slice(0, 16) ?? '',
            end_date:     editTarget.end_date?.slice(0, 16) ?? '',
            cta_text:     editTarget.cta_text ?? '',
            cta_url:      editTarget.cta_url ?? '',
    ...
    <div className="space-y-4">
      ...
      {showForm && (
        <form onSubmit={save} className="p-5 bg-white border border-gray-200 rounded-2xl shadow-sm space-y-4">
          <div className="flex items-center justify-between mb-1">
            <h3 className="font-bold text-gray-900 text-sm">{editId ? 'Edit Announcement' : 'New Announcement'}</h3>
            <button type="button" onClick={() => setShowForm(false)} className="p-1 hover:bg-gray-100 rounded-lg text-gray-400">
              <X className="w-4 h-4" />
            </button>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div key={a.id} className="flex items-start gap-4 p-4 bg-white border border-gray-200 rounded-xl hover:shadow-sm transition-shadow">
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                <button onClick={() => toggle(a)}
                  className={`p-2 rounded-lg transition-colors ${a.is_active ? 'bg-green-100 text-green-600 hover:bg-green-200' : 'bg-gray-100 text-gray-400 hover:bg-gray-200'}`}>
                  {a.is_active ? <Eye className="w-4 h-4" /> : <EyeOff className="w-4 h-4" />}
                </button>
                <button onClick={() => openEdit(a)} className="p-2 bg-blue-100 text-blue-600 rounded-lg hover:bg-blue-200">
                  <Edit3 className="w-4 h-4" />
...
const Advanced: React.FC = () => (
  <div className="space-y-6 p-6 bg-gray-50 min-h-screen">
    ...
    <Section
      ...
      subtitle="Enable or disable student, teacher, and parent portals"
    >
      <PortalSettings />
    </Section>
    ...
    <Section
      ...
      subtitle="Create banners, carousel slides, and scrolling ribbon messages for your landing page"
    >
      <EventManagement />
    </Section>
    ...
    <Section
      ...
      title="Announcements"
      subtitle="Send notices to students, teachers, and parents inside the portal"
    >
      <AnnouncementsManager />
    </Section>
  </div>
