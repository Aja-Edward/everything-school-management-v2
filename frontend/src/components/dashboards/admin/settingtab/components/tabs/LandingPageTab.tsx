...
const SectionEditor: React.FC<{
  ...
  // Local state for phone list so empty new entries survive between renders
  const [phones, setLocalPhones] = useState<string[]>(() => {
    const raw = section.contact_phone ?? '';
    return raw ? raw.split('\n') : [];
  });

  const upd = (patch: Partial<LandingSection>) => onChange({ ...section, ...patch });

  const setPhones = (updated: string[]) => {
    setLocalPhones(updated);
    // Only persist non-empty entries to parent; empty rows are kept locally so the input stays visible
    upd({ contact_phone: updated.filter(p => p.trim()).join('\n') });
  };
