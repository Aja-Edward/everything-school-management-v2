      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 overflow-hidden">
        ...
        <div className="px-6 pt-6 pb-4 border-b border-gray-100">
          ...
          <p className="mt-1 text-sm text-gray-500">
            Moves every <strong className="font-medium text-gray-700">Promoted</strong> student
            {shown && (
              <>
                {" "}in <strong className="font-medium text-gray-700">{shown.from_class.name}</strong>{" "}
                ({sessionName}) into{" "}
                <strong className="font-medium text-gray-700">{shown.to_class.name}</strong>
              </>
            )}
            .
          </p>
            ...
            <>
              {applied ? (
                <p className="text-sm text-green-800 bg-green-50 border border-green-200 rounded-lg p-3">
                  {count} student{count !== 1 ? "s" : ""} moved to {applied.to_class.name}.
                </p>
              ) : count === 0 ? (
                <p className="text-sm text-gray-600 bg-gray-50 rounded-lg p-3">
                  No promoted students are waiting to move. Run auto-promotion or
                  override flagged students first.
                </p>
              ) : (
                <div className="bg-indigo-50 border border-indigo-100 rounded-lg p-4 text-sm text-indigo-800">
                  <p className="font-medium">
                    {count} student{count !== 1 ? "s" : ""} will move to {shown.to_class.name}
                  </p>
              ...
              {unresolved > 0 && (
                <p className="text-xs text-amber-800 bg-amber-50 border border-amber-200 rounded p-2">
                  ...
                  decide on them, apply promotions again — students already moved are not affected.
                </p>
              )}

              {!applied && count > 0 && shown.moved.some((m) => !m.section) && (
